...
- SPEC-02 (the *Meld interesse* page and the privacy policy page the consent text links to).
...
- Placeholders, recorded in INPUTS-NEEDED: the data-controller identity (legal name, org. nr, contact) shown in the consent and policy copy, and the live plot identifiers. The privacy policy is marked *bør gjennomgås av jurist eller DPO*. No client personal data is seeded; only synthetic test data and test e-post addresses are used in development.
