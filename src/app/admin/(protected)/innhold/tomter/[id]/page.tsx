...
import { getPlot, listVersions } from "@/lib/content/service";
...
export default async function EditPlot({
  params,
  searchParams,
}: {
  ...
  if (!plot) notFound();
  const versions = await listVersions(db, "plot", id);
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-foreground text-2xl font-semibold tracking-tight">{plot.label}</h1>
            ...
            {versions.map((v) => (
              <li key={v.id} className="flex items-center justify-between py-2 text-sm">
                ...
                <form action={restoreVersionAction}>
                  ...
                  <input type="hidden" name="back" value={`/admin/innhold/tomter/${id}`} />
